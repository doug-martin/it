...
exports.__defineSetter__("reporter", function (f) {
    ...
    } else if (comb.isObject(f) && isFunction(f.printTitle) && isFunction(f.printSuccess) && isFunction(f.printError) && isFunction(f.printSummary)) {
        formatter = f;
    } else {
...
function describeIt(description, sub, level) {
    var it = {
        ...
        _processSummary: function (summary, promise, method) {
            ...
            if (!sub) {
                formatter.printSummary(summary.summaries, summary.duration);
            }
            promise[method](ret);
        },
...
function printSummary(its) {
    ...
    var summary = {};
    var keys = Object.keys(its), length = 0;
