...
var it = {
    ...
    reporter: function reporter(r) {
        var formatter;
        if (comb.isString(r)) {
            formatter = formatters[r];
        } else if (comb.isObject(r) && isFunction(r.printTitle) && isFunction(r.printSuccess) && isFunction(r.printError) && isFunction(r.printSummary)) {
            formatter = r;
        } else {
            throw new Error("Invalid formatter " + f);
        }
        interfaces.reporter(formatter);
    },
     ...
     */
    run: function run() {
        return serial(Object.keys(interfaces).map(function (i) {
            return interfaces.run;
        })).chain(function () {
                return interfaces.printSummary();
            });
    }
