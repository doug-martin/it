...
function reportCoverageTable(cov, out) {
    ...
    // Source
    body = body.concat(cov.files.map(getFileSource));
    writeFile(body.join("\n"), out);
}
...
function getFileFormatter(file) {
    return function formatSource(line, i) {
        i += 1;
        var hits = string.pad(file[i] === 0 ? 0 : (file[i] || ' '), null, true), css = "green", hasHits = file[i] === 0 ? false : true;
        return string.format("<li class='%s'><span class='%s'>%s</span><code>%s</code></li>", hasHits ? 'green' : 'red', file[i] === undefined ? "" : hasHits ? "hits" : "nohits", hits, line);
    };
}
...
function populateCoverage(cov) {
    ...
    for (var name in cov) {
        ...
        if (comb.isArray(file)) {
            ...
            file.source = format("<pre class='prettyprint linenums'><ol class='linenums'>%s</ol></pre>", file.source.map(getFileFormatter(file)).join('\n'));
